...
def txt(s, items, x, y, w, h, sz=15, boxed=False):
    """Content text — uses Calibri 15pt default"""
    if boxed:
...
def slide_lr(title, items, img_name, img_folder=MY, sz=14):
...
def slide_full(title, items, sz=15):
    s = S(); bg(s); hdr(s, title); print(f"  {n}. {title[:42]}")
...
print("Building V11 — Adaptive Multi-Modal Satellite Image Super-Resolution...")
...
tf = tb.text_frame; tf.word_wrap = True
for line in ["Adaptive Multi-Modal", "Satellite Image", "Super-Resolution Using", "Hybrid Deep Learning"]:
    p = tf.paragraphs[0] if not tf.paragraphs[0].text else tf.add_paragraph()
    p.text = line; p.font.size = Pt(36); p.font.bold = True; p.font.color.rgb = WH; p.font.name = FONT
...
slide_full("Problem Statement", [
    "Most open-source data have coarse spatial resolution, and it is",
    "advantageous to use different sensors for effective enhancement;",
    "however, the sensors have differences in spatial resolution, spectral",
    "characteristics, noise levels, and often lack spatial or temporal overlap.",
], sz=18)

# ══════════════════════════════════════════
# 6b. OBJECTIVE
# ══════════════════════════════════════════
slide_full("Objective", [
    "• To develop a multisensor SR (super-resolution) approach that",
    "  effectively addresses the characteristic differences of different sensors.",
    "",
    "• Means to design a method that combines multiple sensors and",
    "  improve image resolution, while properly handling differences in",
    "  spatial resolution, spectral properties, noise and acquisition conditions."
], sz=18)
...
# SAVE
# ══════════════════════════════════════════
out = os.path.join(BASE, "PPTs", "SRMamba_T_BTP_v11.pptx")
prs.save(out)
print(f"\n>>> Saved: {out}")
